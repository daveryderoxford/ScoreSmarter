...
export async function updateRaceResultsSheetImagePath(
  ...
  const raceDocumentPath = raceDocPath(clubId, raceId);
  const db = getFirestore();
  logScan(requestId, "update_race_doc", "Updating race with results sheet image path", {
    raceDocPath: raceDocumentPath,
    storagePath,
  });

  await db.doc(raceDocumentPath).set(
    {
...
export function resultsSheetStoragePath(clubId: string, raceId: string): string {
  return `clubs/${clubId}/results-sheets/scan-img-${raceId}.jpg`;
}

export function raceDocPath(clubId: string, raceId: string): string {
  return `clubs/${clubId}/races/${raceId}`;
}